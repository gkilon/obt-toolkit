...
export const Landing: React.FC = () => {
  ...
  const [registrationCode, setRegistrationCode] = useState(() => localStorage.getItem('obt_vip_code') || ''); 
  const [isLoading, setIsLoading] = useState(false);
  const [isGoogleLoading, setIsGoogleLoading] = useState(false);
  const [error, setError] = useState('');
  const [successMsg, setSuccessMsg] = useState('');

  const [hasAccess, setHasAccess] = useState(() => localStorage.getItem('obt_vip_access') === 'true');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (view === 'register') {
        const codeToUse = registrationCode || localStorage.getItem('obt_vip_code') || '';
        await storageService.registerUser(name, email, password, codeToUse);
        navigate('/dashboard');
      } else if (view === 'login') {
        ...
        const codeToUse = registrationCode || localStorage.getItem('obt_vip_code') || '';
        await storageService.resetPassword(email, codeToUse, password);
  ...
  const handleGateSubmit = async (e: React.FormEvent) => {
    ...
    setIsVerifying(true);
    try {
      const isValid = await storageService.validateRegistrationCode(gateCode);
      if (isValid) {
        localStorage.setItem('obt_vip_access', 'true');
        localStorage.setItem('obt_vip_code', gateCode);
        setRegistrationCode(gateCode);
        setHasAccess(true);
      } else {
        setError(lang === 'he' ? 'קוד הגישה שגוי.' : 'Invalid access code.');
      }
    } catch (err: any) {
      setError(lang === 'he' ? 'שגיאה באימות הקוד.' : 'Error validating code.');
    } finally {
            ...
            <div className="glass-panel w-full max-w-[460px] p-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
                <div className="mb-10 text-center">
                    <h2 className="text-2xl font-medium text-white">
                        {lang === 'he' ? 'כניסה מאובטחת' : 'Secure Access'}
                    </h2>
                    <p className="text-sm text-white/40 mt-2">
                        {lang === 'he' ? 'אנא הזן קוד גישה ארגוני כדי להמשיך' : 'Please enter your organizational access code to continue'}
                    </p>
                ...
                <form onSubmit={handleGateSubmit} className="space-y-6">
                    <input 
                      ...
                      className="dark-input text-center tracking-widest text-xl" 
                    ...
                    <Button type="submit" variant="primary" className="w-full text-lg py-4" isLoading={isVerifying}>
